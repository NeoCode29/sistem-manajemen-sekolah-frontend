...
export function useSemesters() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Gagal memuat data semester'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createSemester = async (payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menyimpan semester'));
    }
  };
  ...
  const updateSemester = async (id: string, payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal memperbarui semester'));
    }
  };
  ...
  const toggleSemesterActive = async (id: string) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal mengubah status semester'));
    }
  };
  ...
  const deleteSemester = async (id: string) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menghapus semester'));
    }
  };
