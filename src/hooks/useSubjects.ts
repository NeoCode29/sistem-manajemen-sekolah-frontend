...
export function useSubjects() {
  ...
  const createSubject = async (payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menyimpan mata pelajaran'));
    }
  };
  ...
  const updateSubject = async (id: string, payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal memperbarui mata pelajaran'));
    }
  };
  ...
  const deleteSubject = async (id: string) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menghapus mata pelajaran'));
    }
  };
