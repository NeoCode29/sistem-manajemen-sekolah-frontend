...
export function useAcademicYears() {
  ...
  const fetchYears = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Gagal memuat data tahun ajaran'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createAcademicYear = async (payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menyimpan tahun ajaran'));
    }
  };
  ...
  const updateAcademicYear = async (id: string, payload: any) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal memperbarui tahun ajaran'));
    }
  };
  ...
  const toggleAcademicYearActive = async (id: string) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal mengubah status tahun ajaran'));
    }
  };
  ...
  const deleteAcademicYear = async (id: string) => {
    try {
      ...
      throw new Error(getErrorMessage(err, 'Gagal menghapus tahun ajaran'));
    }
  };
